// translate.go — 角色转译：自然语言 → 多模态生图提示词扩写。
//
// 角色转译：把用户输入的描述转成可直接出图的结构化提示词。
// 输入角色/场景的简短描述，调用 OpenAI 兼容聊天接口，按「视觉导演助手」系统提示
// 扩写为 {总提示词, 角色描述, 场景描述, 负面词, 推荐尺寸} 的 JSON。
...
// translatePresetVersion 每次更新 translateSystemPrompt 内容时 +1（前端按此版本号刷新缓存）。
const translatePresetVersion = 3

const translateSystemPrompt = `你是一个专为多模态生图模型（如 Qwen/FLUX/DALL-E）提供提示词扩写的视觉导演助手。
...
// translateRefused 模型拒答或两次尝试均未返回合法 JSON。
type translateRefused struct{ msg string }

func (e *translateRefused) Error() string { return e.msg }

var (
	jsonObjRe    = regexp.MustCompile(`(?s)\{.*\}`) // (?s) = DOTALL：容忍多行 JSON
	commaSplitRe = regexp.MustCompile(`[,，]`)
)

// mergeNegative 模型负面词与基础防崩词去重合并（大小写不敏感，模型词在前）。
func mergeNegative(modelNeg string) string {
	var items []string
	seen := map[string]bool{}
	add := func(raw string) {
		for _, t := range commaSplitRe.Split(raw, -1) {
...
func parseTranslateJSON(content string) (*translateResult, error) {
	...
	if m == "" {
		return nil, errors.New("模型未返回 JSON 对象")
	}
	var data struct {
		Prompt          string `json:"prompt"`
		CharacterPrompt string `json:"character_prompt"`
	...
	r := &translateResult{
		...
		CharacterPrompt: strings.TrimSpace(data.CharacterPrompt),
		MainPrompt:      strings.TrimSpace(data.MainPrompt),
		Width:           832, Height: 1216, Steps: 28, CfgScale: 7.0,
	}
	neg := strings.TrimSpace(data.NegativePrompt)
	if neg == "" {
		neg = translateDefaultNegative
	}
	r.NegativePrompt = mergeNegative(neg)
	if data.Params.Width > 0 {
		r.Width = data.Params.Width
	}
	if data.Params.Height > 0 {
		r.Height = data.Params.Height
	}
	if data.Params.Steps > 0 {
		r.Steps = data.Params.Steps
	}
	if data.Params.Cfg > 0 {
		r.CfgScale = data.Params.Cfg
	}
	if r.Prompt == "" && r.CharacterPrompt == "" && r.MainPrompt == "" {
		return nil, errors.New("JSON 中 prompt / character_prompt / main_prompt 均为空")
	}
	return r, nil
}

// translateCharacter 调用 OpenAI 兼容聊天接口做角色转译（防拒答重试一次）。
func translateCharacter(ctx context.Context, tgt qwenTarget, userText string) (*translateResult, error) {
	userText = strings.TrimSpace(userText)
	if userText == "" {
		return nil, errors.New("请输入角色或场景描述")
	}
	var lastErr error
	for attempt := 1; attempt <= 2; attempt++ {
		sysMsg := translateSystemPrompt
		if attempt == 2 {
		...
		raw, status, err := postJSON(ctx, tgt, "/chat/completions", body, 180*time.Second)
		if err != nil {
			lastErr = err
			continue
		}
		if status < 200 || status >= 300 {
			lastErr = fmt.Errorf("转译接口返回 HTTP %d：%s", status, snippet(raw, 200))
			continue
		}
		var cr struct {
			Choices []struct {
		...
		if err := json.Unmarshal(raw, &cr); err != nil || len(cr.Choices) == 0 {
			lastErr = errors.New("转译响应缺少 choices（片段：" + snippet(raw, 150) + "）")
			continue
		}
		r, perr := parseTranslateJSON(cr.Choices[0].Message.Content)
		if perr == nil {
			return r, nil
		}
		lastErr = fmt.Errorf("%w（模型回复片段：%s）", perr,
			snippet([]byte(cr.Choices[0].Message.Content), 200))
	}
	return nil, &translateRefused{msg: fmt.Sprintf("转译失败（模型拒答或格式错误）：%v", lastErr)}
}
